...
export async function sendContactEmail(formData: FormData) {
 ...
 console.log("API",process.env.RESEND_API_KEY)
  ...
  const isLikelyValid = apiKey.startsWith("re_")

  console.log("isLikelyValid",isLikelyValid)
  ...
  if (!isLikelyValid) {
    ...
    return {
      ...
      message:"Email service isn’t configured yet. Please add a valid RESEND_API_KEY in your Vercel dashboard (Project → Settings → Environment Variables) and redeploy.",
    }
  }
  ...
  try {
    ...
    const { data, error } = await resend.emails.send({
      ...
      to: ["test01.shagufta@gmail.com"],
      reply_to: email, // This allows Shagufta to reply directly to the sender
...
Reply directly to this email to respond to ${name}.
    ...
    return {
      ...
      mailtoLink: `mailto:shaguftarasheed21@gmail.com?subject=${encodeURIComponent(
